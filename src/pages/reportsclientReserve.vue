<template>
    <q-page>
        <q-table title="Client Reservation Reports" :data="MostEvent" :columns="columns" :filter="filter" row-key="name">
                <template v-slot:body="props">
                    <q-tr>
                        <q-td key="clientFullName" :props="props">{{props.row.checkerName}}</q-td>
...
export default {
    data(){
        return{
            ...
            Food: [],
            columns: [
                { name: 'clientFullName', align: 'left', required: true, label: 'Client Name', field: 'clientFullName', sortable: true },
                { name: 'count', align: 'right', label: 'Client Ranking', sortable: true ,field: 'count'},
                { name: 'ordered', align: 'right', label: 'Number of Times Client Book an Event', sortable: true }
            ]
        }
    },
    mounted(){
    ...
    computed: {
        ...
        MostEvent(){
            ...
            let optionss = this.$lodash.filter(this.Reservation, m => {
               fullName.push({Event: m.clientFullName})
            })
            console.log(fullName, 'fullName')
            let count = this.$lodash.countBy(fullName,'Event')
